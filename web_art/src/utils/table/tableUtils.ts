...
function toPaginationNumber(value: unknown): number | undefined {
...
function uniquePaginationFields(primaryKey: string, aliases: string[]): string[] {
  return Array.from(new Set([primaryKey, ...aliases]))
}

function pickPaginationValue(obj: Record<string, unknown>, fields: string[]): number | undefined {
  for (const field of fields) {
    const value = toPaginationNumber(obj[field])
    if (value !== undefined) {
      return value
    }
  }
...
export function normalizePaginationRequestParams<T extends Record<string, unknown>>(
  params: T,
  currentKey = tableConfig.paginationKey.current,
  sizeKey = tableConfig.paginationKey.size
): T {
  const normalized = { ...params } as Record<string, unknown>
  const currentFields = uniquePaginationFields(currentKey, tableConfig.requestCurrentFields)
  const sizeFields = uniquePaginationFields(sizeKey, tableConfig.requestSizeFields)

  const currentValue = pickPaginationValue(normalized, currentFields)
  const sizeValue = pickPaginationValue(normalized, sizeFields)
  ...
  if (sizeValue !== undefined) {
    normalized[sizeKey] = sizeValue
  }

  currentFields
    .filter((field) => field !== currentKey)
    .forEach((field) => delete normalized[field])

  sizeFields
    .filter((field) => field !== sizeKey)
    .forEach((field) => delete normalized[field])
