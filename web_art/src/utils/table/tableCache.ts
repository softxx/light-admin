...
export class TableCache<T> {
  ...
  private generateTags(params: Record<string, unknown>): Set<string> {
    ...
    const paginationFields = new Set([
      ...tableConfig.requestCurrentFields,
      ...tableConfig.requestSizeFields,
      'total'
    ])
    ...
    // 添加分页标签
    const sizeField = tableConfig.requestSizeFields.find((field) => params[field] !== undefined)
    const sizeValue = sizeField ? params[sizeField] : 10
    tags.add(`pagination:${String(sizeValue || 10)}`)
    // 添加通用分页标签，用于清理所有分页缓存
    tags.add('pagination')
