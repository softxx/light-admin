...
export class MenuProcessor {
  ...
  /**
   * Merge local hidden support pages into backend-controlled routes,
   * so utility pages like user center can still be opened directly.
   */
  private mergeLocalHiddenRoutes(menuList: AppRouteRecord[]): AppRouteRecord[] {
    const existingPaths = this.collectRoutePaths(menuList)
    const supportRoutes = this.collectLocalHiddenRoutes(routeModules).filter(
      (route) => route.path && !existingPaths.has(route.path)
    )

    return [...menuList, ...supportRoutes]
  }

  private collectRoutePaths(
    ...
    routes.forEach((route) => {
      ...
      if (route.children?.length) {
        this.collectRoutePaths(route.children, pathSet)
      }
    })
  ...
  private collectLocalHiddenRoutes(
    routes: AppRouteRecord[],
    parentPath = ''
  ): AppRouteRecord[] {
    return routes.reduce<AppRouteRecord[]>((accumulator, route) => {
      ...
      if (route.meta?.isHide === true && route.component) {
        accumulator.push({
          ...route,
          path: fullPath,
          children: undefined
      ...
      if (route.children?.length) {
        accumulator.push(...this.collectLocalHiddenRoutes(route.children, fullPath))
      }
  ...
  /**
   * Ensure backend mode always has an accessible home page.
   */
  private ensureDefaultHome(menuList: AppRouteRecord[]): AppRouteRecord[] {
    const hasHome = menuList.some((item) => this.containsDashboardRoute(item))
  ...
  private containsDashboardRoute(route: AppRouteRecord): boolean {
    if (route.component === '/dashboard/console') {
      return true
    }
