...
import {
  ...
  extractTableData,
  normalizePaginationRequestParams,
...
function useTableImpl<TApiFn extends (params: any) => Promise<any>>(
  ...
  const normalizedApiParams = normalizePaginationRequestParams(
  ...
  const fetchData = async (
    ...
    try {
      let requestParams = normalizePaginationRequestParams(
        Object.assign(
          {},
          searchParams,
          {
  ...
  const resetSearchParams = async (): Promise<void> => {
    ...
    // 重新设置默认参数
    Object.assign(searchParams, normalizedApiParams, defaultPagination)
