  ...
  <div class="art-table" :class="{ 'is-empty': isEmpty }" :style="containerHeight">
    <ElTable ref="elTableRef" v-loading="!!loading" v-bind="mergedTableProps">
      <template v-for="col in columns" :key="col.prop || col.type">
        ...
        <!-- 渲染普通列 -->
        <ElTableColumn
          v-else-if="col.type === 'selection' || col.type === 'index'"
        ...
        <ElTableColumn v-else v-bind="cleanColumnProps(col)">
          ...
          <template #default="slotScope">
            ...
            <TableCellRenderer
              ...
              :content="getColumnContent(col, slotScope.row)"
            />
          </template>
        </ElTableColumn>
      </template>
...
<script setup lang="ts">
  ...
  // 分页大小变化
  const getColumnContent = (col: ColumnOption, row: Record<string, any>) => {
    if (col.formatter) {
